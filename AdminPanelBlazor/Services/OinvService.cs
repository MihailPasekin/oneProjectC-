    ...
    public class OinvService : PageModel, IOinvService
    {
        ...
        public async Task<ListOINV> GetInvoices()
        {
            using (HttpResponseMessage response = await httpClient.GetAsync("/api/Invoice/getinvoicebyexpeditor?uAgent=885"))
            {
                ...
                string apiResponse = await response.Content.ReadAsStringAsync();
                listOINV.ListOinvs = JsonConvert.DeserializeObject<List<InvoiceOinv>>(apiResponse) ?? new List<InvoiceOinv>(); ;
                return listOINV;
            }
        }
        ...
        public async Task<ListOINV> GetInvoiceByDocNumber(string docNumber)
        {
            using (HttpResponseMessage response = await httpClient.GetAsync($"/api/Invoice/getinvoicebydocnumber?docnumber={docNumber}"))
            {
                ListOINV listOINV = new ListOINV();
                string apiResponse = await response.Content.ReadAsStringAsync();
                listOINV.ListOinvs = JsonConvert.DeserializeObject<List<InvoiceOinv>>(apiResponse) ?? new List<InvoiceOinv>(); ;
        ...
        public async Task<List<ORDR>> GetSapOrdersByRegion(string region)
        {
            List<ORDR> tempOrderList;

            using (HttpResponseMessage response = await httpClient.GetAsync($"/api/saporder/getallopensaporderbyregion?region={region}"))
        ...
        public async Task OnPostCreateDelivery(List<int> selectedOrders)
        {
            foreach (int orderId in selectedOrders)
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (HttpResponseMessage response = await httpClient.PostAsync("http://192.168.0.70:5093/api/Delivery/createdelivery", body))
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
                    }
        ...
        {
            using (HttpResponseMessage response = await httpClient.GetAsync($"/employe/getemployes"))
            {
                string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<List<OHEM>> GetAllEmployesEkspeditorAsync(string region)
        {
            using (HttpResponseMessage response = await httpClient.GetAsync($"/api/SapEmploye/getsapemployesbyjobtitleregion?jobTitle=Sürüji ekspeditor&region={region}"))
            {
                string apiResponse = await response.Content.ReadAsStringAsync();
